...
Просмотр и кнопки **модалок**: редактирование профиля (ФИО, рост, вес, телефон, город, о себе, дата рождения, AI summary) и **привязка Telegram** (одноразовая ссылка, отвязка). Сохранение профиля через `PUT /users/me`. При изменении веса/роста/возраста клиент может синхронизировать связанные ключи в доп. полях всех ассистентов (`syncLinkedAiExtrasFromProfileBody`).
...
**Привязка аккаунта:** в профиле веб-приложения создаётся одноразовая ссылка (`POST /users/me/telegram/link`); после перехода в Telegram и `/start` с токеном в БД сохраняется `users.telegram_chat_id`. **Вес из бота** записывается в `user_weight_entries` для привязанного пользователя (та же логика, что и через API). Отвязка: `DELETE /users/me/telegram`.

**Состояние и данные:** флаг «ждём вес» хранится в **`TelegramChatStateStore`** (в памяти процесса API). При горизонтальном масштабировании нескольких инстансов нужен общий стор для диалога или sticky-сессии к webhook.
