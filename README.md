...
Веб-приложение: **тренировки** (программы, каталог упражнений, запись подходов, история), **дневник веса**, **профиль**, **AI-чат** с настраиваемыми ассистентами и **админка**; опционально — **Telegram-бот** (webhook, сценарий «отправить вес»). Репозиторий: **HabiHamAIAPI** (ASP.NET Core, PostgreSQL, JWT) и **HabiHamAIUi** (React, Vite).
...
Интеграция опциональна: библиотека **Telegram.Bot**, обработчик `ITelegramUpdateHandler`, входящий POST на **`/api/telegram/webhook`** (см. § 4.10).

**Включение:** задайте `TELEGRAM_BOT_TOKEN` или `Telegram:BotToken` в конфигурации или окружении. Без токена `ITelegramBotClient`, обработчик апдейтов и фоновая регистрация webhook не регистрируются в DI.

**Webhook:** если задан `TELEGRAM_PUBLIC_BASE_URL` или `Telegram:PublicBaseUrl` (публичный HTTPS-адрес этого API, без `/` в конце), при старте вызывается **`setWebhook`** на `{PublicBaseUrl}/api/telegram/webhook`.

**Меню команд:** при наличии токена при старте вызывается **`SetMyCommands`** — в клиенте Telegram отображаются команды (`/start`, `/weight`, `/help`, `/keyboard`, `/hide` и др.).

**Клавиатура и сценарии:**

- После **`/start`** пользователю показывается приветствие и reply-клавиатура: **«Отправить вес»**, **«Помощь»**, **«Скрыть клавиатуру»**.
- **`/weight`** или кнопка «Отправить вес» переводят чат в режим **ожидания веса**: бот просит отправить **одно число** в килограммах (допустимы точка или запятая как разделитель дробной части). Диапазон **0.01–700** кг (как разумные границы для ввода). Любой другой текст в этом режиме отклоняется с подсказкой.
- **«Отмена»** (в режиме ввода веса), **`/start`**, **`/hide`** или «Скрыть клавиатуру» сбрасывают ожидание веса и возвращают обычную клавиатуру (где применимо).

**Состояние и данные:** флаг «ждём вес» хранится в **`TelegramChatStateStore`** (в памяти процесса API). При горизонтальном масштабировании нескольких инстансов нужен общий стор. **Запись веса в PostgreSQL из бота не выполняется** — в модели пользователя нет привязки к Telegram; подтверждение веса отправляется только в чат. Для сохранения в `user_weight_entries` потребуется связь аккаунта с `chat_id` и вызов существующей логики (`UsersService` и т.п.).
